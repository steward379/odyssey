...
app.get("/squared/:integer", (req, res) => {
  const integer = parseInt(req.params.integer, 10);
...
app.use(function (err, req, res, next) {
  ...
  let message = encodeURIComponent("0 - Unknown error occurred!");
  res.status(500).redirect("/error?message=" + message);
  // let messages = [{ type: "text", content: message }];
  // res.status(500).render("error", { message: message, messages: messages });
});
